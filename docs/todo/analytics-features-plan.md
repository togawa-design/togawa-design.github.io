...
**実装場所**: 管理画面 → 広告費用管理セクション

**実装機能**:
- 広告費用入力（会社・月・チャンネル別）
- 広告費用一覧（過去データのフィルター表示）
- ROIレポート（CPA計算・チャンネル別分析）
- CSVインポート（Google/TikTok/X Ads対応）
- 投資判断サポート（推奨アクション生成）

**データソース**: `ad_costs`コレクション + `applications`コレクション（UTM経由）
