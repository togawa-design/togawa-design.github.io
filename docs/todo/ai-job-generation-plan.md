...
|-------|------|----------------------|
| Phase 1 | 説明文生成、タイトル提案、キャッチコピー、文章校正、NGワード検出 | 15-30回 |
...
| **文章校正** | 誤字脱字・表現チェック | 品質向上 | 1 |
| **NGワード検出** | 法令違反・差別表現の検出 | リスク回避 | 1 |
...
**エンドポイント**: `/checkJobText`

#### チェック項目

| カテゴリ | チェック内容 | 例 |
|----------|-------------|-----|
| **誤字脱字** | タイポ、変換ミス | 「勤むる」→「勤める」 |
| **表記ゆれ** | 統一されていない表現 | 「お問合せ」と「お問い合わせ」混在 |
| **冗長表現** | 不要に長い表現 | 「することができます」→「できます」 |
| **敬語ミス** | 二重敬語、敬語の誤用 | 「ご覧になられる」→「ご覧になる」 |
| **句読点** | 読点の過不足 | 読みにくい長文の指摘 |

#### NGワード検出

| カテゴリ | 検出ワード例 | リスク |
|----------|-------------|--------|
| **年齢制限** | 「若い方歓迎」「35歳まで」 | 雇用対策法違反 |
| **性別制限** | 「男性のみ」「女性限定」 | 男女雇用機会均等法違反 |
| **差別表現** | 特定の属性を排除する表現 | 人権問題 |
| **誇大表現** | 「絶対」「必ず」「100%」 | 景品表示法違反 |
| **給与詐称** | 実態と異なる金額表記 | 職業安定法違反 |
...
問題がなければ「チェック完了：問題ありません」と出力。
```

**NGワード検出**:

```
以下の求人文章に、法令違反や差別表現が含まれていないかチェックしてください。
...
問題がなければ「チェック完了：法令上の問題は検出されませんでした」と出力。
```
...
6. [ ] プロンプトテンプレート作成
7. [ ] フロントエンドUI実装
8. [ ] 文章校正API作成（checkJobText）
9. [ ] NGワードリスト作成・プロンプト調整
10. [ ] 校正結果UI実装（インライン表示）
11. [ ] テスト・調整
12. [ ] 本番デプロイ
